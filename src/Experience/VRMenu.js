...
export default class VRMenu extends THREE.Group {
  ...
  mainDefinitions() {
    return [
      ...
      this.button("ortho", "Ortho on terrain", () => this.world.toggleActiveScanOrtho(), () => this.world.activeScan()?.orthoOnTerrain ? "ON" : "OFF", () => Boolean(this.world.activeScan()?.record.ortho)),
  ...
  modelDefinitions() {
    ...
    const definitions = [this.title(scan ? `MODEL · ${scan.record.name}` : "CHOOSE SCAN MODEL")];
    if (!scan) {
      definitions.push(this.hint("Choose a scan flight first"));
    } else if (!scan.model) {
      definitions.push(this.hint("No 3D model for this flight"));
    } else {
